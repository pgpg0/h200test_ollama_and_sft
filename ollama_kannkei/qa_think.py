...
INPUT_JSONL_PATH = Path('/home/ubuntu/client/Data_azami/code/chat_sft_data_qa.jsonl')
OUTPUT_JSONL_PATH = Path('/home/ubuntu/client/Data_azami/code/makedata/thinking_results_qa.jsonl')
FAILED_JSONL_PATH = Path('/home/ubuntu/client/Data_azami/code/makedata/failed_data_qa.jsonl')
...
# === プロンプト（ユーザー指定のまま） ===
THINKING_PROMPT = """
あなたはB2B向け製品の営業担当として、顧客からの問い合わせに対して丁寧かつ正確に回答します。
いただいた質問（question）と、その回答例（answer）が与えられます。
あなた自身が質問に対して提示した回答をした場合どのような思考回路になるかを考えて提示してください。

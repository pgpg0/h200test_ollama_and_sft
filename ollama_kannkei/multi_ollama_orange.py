...
def main():
    ...
    for csv_path in csv_files:
        ...
        for _, row in df.iterrows():
            product_name = row.get('タイトル', '名称不明')
            features_list = []
            if row.get('clistdisc'): features_list.append(str(row['clistdisc']))
            if row.get('clistdisc4'): features_list.append(str(row['clistdisc4']))
            if row.get('内容5'): features_list.append(str(row['内容5']))
            if row.get('clistinlinebadge') and row.get('clistinlinebadge6'):
                features_list.append(f"{row['clistinlinebadge']}: {row['clistinlinebadge6']}")
            features_string = "・" + "\n・".join(filter(None, features_list))
            product_info = f"商品名: {product_name}\n特徴:\n{features_string}"
            base_result_data = {"product_name": product_name, "manufacturer": row.get('cproduct_txt_read', 'N/A'), "features_text": features_string}
    ...
    print(f"Format 2: Total products to process: {len(all_products)}")

    # 並列処理と結果出力 (フォーマット1と共通)
